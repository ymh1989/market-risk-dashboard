...
def _baseline_comparison(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    comparisons = [
        ...
        ("5D 하방", "downside_5d", "brier", "낮을수록 우수"),
        ("5D 하방", "downside_5d", "auc", "높을수록 우수"),
        ("5D 하방", "downside_5d", "recall", "높을수록 우수"),
        ("20D 하방", "downside_20d", "brier", "낮을수록 우수"),
        ("20D 하방", "downside_20d", "auc", "높을수록 우수"),
        ("20D 하방", "downside_20d", "recall", "높을수록 우수"),
...
def _latest_summary(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            ...
            {"항목": "5D 하방확률", "값": _fmt(latest["prob_downside_5d"])},
            {"항목": "20D 하방확률", "값": _fmt(latest["prob_downside_20d"])},
