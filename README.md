...
`파도` 모드는 Three.js와 GPU 셰이더로 만든 전체 화면 3D 바다입니다. 네 개의 Gerstner 파동을 합성하되 주 파동의 방향을 왼쪽에서 오른쪽으로 고정했습니다. 포인터가 수면에 닿으면 레이캐스팅으로 해당 3D 좌표를 찾고, 그 주변에만 가우시안 감쇠와 고리형 잔파·반사광을 적용합니다. 파도 모드는 데스크톱과 모바일 모두 전체 화면이며, 눈 모드는 데스크톱에서 최대 1440×900으로 표시됩니다.

품질은 기기의 CPU 코어 수, 메모리 힌트와 화면 크기에 따라 자동으로 선택합니다. 3D 수면 메시 분할은 고화질 220×160, 균형 160×116, 절전 96×72이며 기기별 픽셀 비율도 함께 제한합니다. 모션 감소 환경에서는 정지 상태로 시작하고, Three.js를 실행할 수 없으면 단순 2차원 파도로 전환합니다. 이 장면은 시각적 실시간 시뮬레이션이며 정밀 해양 공학 해석용 모델은 아닙니다.
