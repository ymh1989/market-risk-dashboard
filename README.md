...
메인 화면 기준시각 옆의 작은 눈송이 아이콘을 누르면 독립된 `Snow Lab` 화면이 열립니다. 이 화면은 GPU 기반 2차원 유체장 위에 눈 입자를 합성하며, 마우스나 포인터가 지나갈 때 유체와 입자가 함께 흩어지도록 구성했습니다. 데스크톱에서는 최대 1440×900, 모바일에서는 전체 화면으로 표시됩니다.

품질은 기기의 CPU 코어 수, 메모리 힌트와 화면 크기에 따라 자동으로 선택하며 `고화질`과 `절전` 모드로 직접 바꿀 수도 있습니다. 모션 감소 환경에서는 정지 상태로 시작하고, WebGL을 사용할 수 없으면 눈 입자만 그리는 모드로 전환합니다. 유체 시각화는 CFD 정밀 해석용이 아니라 실시간 상호작용을 위한 Stable Fluids 근사입니다.
