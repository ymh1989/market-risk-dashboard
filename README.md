...
품질은 기기의 CPU 코어 수, 메모리 힌트와 화면 크기에 따라 자동으로 선택합니다. 3D 수면 메시 분할은 고화질 220×160, 균형 160×116, 절전 96×72이며 스펙트럼 성분은 각각 32개, 24개, 16개입니다. 숲의 나무 수는 같은 품질 순서로 1,900그루, 1,250그루, 720그루입니다. 모션 감소 환경에서는 정지 상태로 시작하고, Three.js를 실행할 수 없으면 단순 2차원 장면으로 전환합니다. 스펙트럼 모드는 원문의 512×512 compute-shader FFT를 그대로 복제한 것이 아니라 같은 스펙트럼·분산·변위 수식을 희소 합성한 실시간 시각화이며, 정밀 해양 공학 해석용 모델은 아닙니다.
