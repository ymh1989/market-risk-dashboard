...
현재 맵 아래의 `스트레스 에피소드 리플레이`는 코로나19 급락, 인플레이션·긴축·강달러, AI 반도체 변동성 등 저장된 시장 스트레스 구간을 선택해 5개 기초지수의 시작·시장 정점·종료 경로를 비교합니다. 이 화면은 당시까지 공개된 종가지수로 산식을 재계산한 사후 분석이며, 과거 상품별 내재변동성·상관 스마일·기발행 재고와 실제 헤지 포지션을 복원한 결과는 아닙니다.
