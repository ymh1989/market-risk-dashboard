...
시장 페이지의 `시장 의견 검증 일지`는 AI ROI, 지정학·물가, 금리, 옵션 헤지·디레버리징, 중국 메모리 CAPEX 가설을 관측 근거·판정·운영 조치로 나눕니다. 전망의 방향을 점수에 선반영하지 않으며, 직접 일별 데이터가 없는 중국 메모리 상장·CAPEX는 별도 점수화를 보류하고 데이터 공백을 그대로 표시합니다.
