...
품질은 기기의 CPU 코어 수, 메모리 힌트와 화면 크기에 따라 자동으로 선택합니다. 3D 수면 메시 분할은 고화질 220×160, 균형 160×116, 절전 96×72이며 스펙트럼 성분은 각각 32개, 24개, 16개입니다. 모션 감소 환경에서는 정지 상태로 시작하고, Three.js를 실행할 수 없으면 단순 2차원 파도로 전환합니다. 스펙트럼 모드는 원문의 512×512 compute-shader FFT를 그대로 복제한 것이 아니라 같은 스펙트럼·분산·변위 수식을 희소 합성한 실시간 시각화이며, 정밀 해양 공학 해석용 모델은 아닙니다.

눈 유체장은 `webgl-fluid` 0.3.9, 3D 바다는 Three.js 0.185.1을 사용합니다. 두 라이브러리 모두 MIT 라이선스이며 저장소에 버전을 고정해 배포 시 외부 CDN을 호출하지 않습니다. 라이선스 전문은 `src/vendor/`에 보존합니다.
