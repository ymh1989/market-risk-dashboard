...
홈페이지의 그룹 가중치 카드에는 한글 그룹명과 영문 약칭을 함께 표시합니다. 카드 위에 마우스를 올리거나 정보 버튼에 키보드 초점을 두면 가중 반영 지표의 이름과 현재 점수, 가중치가 0인 관찰 전용 지표를 구분해 볼 수 있습니다. 하단 개별 지표 카드에도 같은 색상과 그룹명을 표시해 그룹 점수와 구성 지표를 바로 연결합니다.
