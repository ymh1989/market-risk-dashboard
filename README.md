...
- 수급·유동성 충격: 시장확산 부담 45%, 기존 `Flow` 30%, `Liquidity` 25%
- 시장확산 부담: 일간 확산도 35%, 5일 평균 45%, 20일 평균 20%를 위험 방향으로 변환
- 거시·신용 확인: 기존 `Macro` 그룹 점수
- 스트레스 가속도: 종합점수·Crash 그룹의 5일 변화와 Breadth 5일 평균 변화
- 펀더멘털 확인: 선행 EPS와 실적 전망 수정폭이 연결되기 전까지 `자료 부족` 표시

가격 충격과 시장 내부 매도 확산이 함께 높고 거시 확인값이 상대적으로 낮으면 `수급성 오버슈팅 가능성`으로 표시합니다. 가격·수급·거시 부담이 동시에 높으면 `시스템 스트레스 주의`로 분류합니다. 이 판정은 원인 해석용이며 기존 종합점수, 6개 그룹 가중치와 경보단계를 바꾸지 않습니다. 현재 `Flow`는 외국인 보유비중 변화 proxy이므로 직접 순매매 데이터로 해석하지 않습니다.
...
증분 갱신은 기존 Parquet의 마지막 거래일 다음 날부터 필요한 날짜만 조회합니다. 예약 작업은 07:30·12:30에는 직전 EOD, 15:35 이후에는 당일 EOD를 시도하며 KRX가 아직 당일 값을 제공하지 않으면 기존 최신일을 보존합니다.
