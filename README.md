...
홈페이지의 `금리·환율·원자재·운임 방향성` 패널은 `data/naver-marketindex-history.json`을 읽어 미국·한국·일본 금리, 원/달러·USD/JPY·USD/CNY, 에너지·금속, 해상운임의 최근 약 3개월 흐름을 보여줍니다. 설정된 자산은 모두 개별 행에 표시하고, 지속성이 확인된 자산만 상단 요약에 올립니다. 금리는 1주·1개월 변화를 bp로, 나머지 자산은 수익률로 표시하며, 최근 일간 10회 또는 주간 6회의 상승·하락 빈도로 방향 지속성을 판독합니다. 원/달러 상승은 원화 약세, USD/JPY 상승은 엔화 약세로 해석합니다.
...
설치 후 LaunchAgent는 지정된 분마다 스크립트를 깨우고, 스크립트가 KST 기준 평일 `07:30`, `12:30`, `15:35`일 때만 실제 갱신을 수행합니다. 현재 작업 폴더에 README나 설정 파일 변경이 남아 있어도 예약 작업이 막히지 않도록, 스크립트는 `origin/main` 기준의 깨끗한 임시 worktree에서 데이터 갱신, ML 재학습, 테스트, JSON 커밋·푸시를 처리합니다.
...
`오늘의 예약 실행`은 평일 세 차례 작업을 완료·진행 중·지연·예정으로 나누어 한 줄에서 비교합니다. 완료된 작업은 완료시각과 실제 소요시간을, 예정 작업은 시작까지 남은 시간을 표시합니다. 최근 성공 이력의 전체 갱신·빠른 갱신 중앙 소요시간을 함께 제공해 평소보다 느린 실행을 판단할 수 있습니다. 주말에는 다음 영업일 예약을 대신 보여줍니다.
