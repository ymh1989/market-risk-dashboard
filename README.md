...
메인 화면 기준시각 옆의 작은 눈송이 아이콘을 누르면 독립된 `Field Lab` 화면이 열립니다. `눈` 모드는 GPU 기반 2차원 유체장 위에 눈 입자를 합성하고, 마우스나 포인터가 지나갈 때 유체와 입자가 함께 흩어집니다. `파도` 모드는 수중 유체장 위에 감쇠 수면 높이장을 합성하며, 포인터 속도와 방향에 따라 파형이 좌우로 전달되고 잔물결이 퍼집니다. 데스크톱에서는 최대 1440×900, 모바일에서는 전체 화면으로 표시됩니다.

품질은 기기의 CPU 코어 수, 메모리 힌트와 화면 크기에 따라 자동으로 선택하며 `고화질`과 `절전` 모드로 직접 바꿀 수도 있습니다. 모션 감소 환경에서는 정지 상태로 시작하고, WebGL을 사용할 수 없으면 2차원 눈 입자 또는 수면 파형만 그리는 모드로 전환합니다. 유체 시각화는 CFD 정밀 해석용이 아니라 실시간 상호작용을 위한 Stable Fluids 근사입니다.

유체장은 MIT 라이선스의 `webgl-fluid` 0.3.9를 로컬에 고정해 사용합니다. 배포 시 외부 CDN을 호출하지 않으며 원본 라이선스는 `src/vendor/webgl-fluid.LICENSE`, `src/vendor/webgl-fluid-origin.LICENSE`에 보존합니다.
