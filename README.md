...
한국 국채 3년·10년은 확정 EOD 시계열과 장중 실시간값을 분리합니다. 차트의 과거 구간과 ML·백테스트 입력에는 확정 EOD만 사용하고, 방향성 패널의 현재값과 전일 변화에는 네이버 실시간 채권 스냅샷을 사용합니다. 당일 잠정 구간은 점선과 `실시간 HH:MM · 잠정` 표식으로 구분합니다. 예약 작업은 전체 계산이 끝난 뒤 게시 직전에 실시간값을 한 번 더 조회합니다.
...
- `data/naver-marketindex-history.json`: 네이버 운임·금속·에너지·채권·국제환율의 확정 이력, 자산별 원천 사용 상태, 한국 3년·10년 금리의 별도 실시간 잠정 스냅샷을 저장합니다.
