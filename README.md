...
`숲` 모드는 절차적으로 만든 산악 지형과 두 겹의 원경 능선 위에 푸른 침엽수를 GPU 인스턴싱으로 배치합니다. 꼭짓점 셰이더가 나무마다 서로 다른 위상과 바람 세기를 적용하므로 수관이 같은 방향으로 복제되어 움직이지 않습니다. 포인터 주변에는 가우시안 감쇠를 적용한 국지 돌풍이 지나가며, 기본 카메라는 눈에 띄지 않을 정도로 천천히 좌우로 이동합니다.
