...
def test_ui_hierarchy_and_accessibility_contract():
    ...
    assert "zoom: var(--desktop-ui-scale);" in "}".join(desktop_scale_rule[:2])
...
def test_summary_exposes_zero_weight_market_shock_decomposition():
    ...
    assert '"점수 보류"' not in app_source
...
def test_operations_page_exposes_daily_schedule_overview():
    ...
    assert 'export INDEX_HISTORY_CACHE_DIR="${INDEX_HISTORY_CACHE_DIR:-$ROOT/data/raw/index_history}"' in overnight_run
...
def test_dashboard_data_requests_bypass_stale_cache():
    ...
    assert 'data-els-performance-range="${option.id}"' in app_source
    assert 'data-els-performance-range-layer="${rangeId}"' in app_source
    assert "function activateElsPerformanceRange" in app_source
    assert "initializeElsPerformanceRangeControls(app)" in app_source
    assert '{ id: "6m", label: "6M", calendarDays: 186 }' in app_source
    assert 'data-chart-domain-start="${timeline.start}"' in app_source
    ...
    assert "const svgDomainStart = Number(svg.dataset.chartDomainStart)" in app_source
    ...
    assert 'height = 52, padding = 6' in app_source
    assert 'r="2.15"' in app_source
    assert "vector-effect: non-scaling-stroke" in styles
    assert "stroke-width: 1.5" in styles
    assert ".market-trend-group--digital .market-trend-row__line" in styles
    ...
    assert 'data-chart-tooltip-host' in app_source
    assert 'svg.closest("[data-chart-tooltip-host]") ?? chart' in app_source
    assert "tooltipHost.append(tooltip)" in app_source
